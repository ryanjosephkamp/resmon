...
- **Reference-manager exports** — any execution's papers export to **BibTeX**, **RIS**, or **CSV** from the interface, alongside the existing Markdown, PDF, and LaTeX report bundle. A fourth format, **JSON**, is available through the API and the MCP server for programs rather than reference managers: it carries the same fields as the CSV export, with a missing value as `null` rather than an empty string. It is not offered in the interface yet, because the interface concatenates several runs into one file and two JSON arrays do not concatenate into valid JSON. Entries with a DOI are emitted as journal articles and those without as generic records; cite keys are made unique within a file and BibTeX special characters are escaped.
