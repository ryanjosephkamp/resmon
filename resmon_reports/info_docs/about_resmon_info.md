...
- Tutorial videos are **linked, never embedded**. The tab renders no third-party frame and issues no request to any YouTube host; opening it costs nothing but the local render. Each section whose `youtubeId` is set carries a `Watch on YouTube` link to `https://www.youtube.com/watch?v=<id>`, and a card at the top of the tab links the whole set as an anonymous temporary playlist (`https://www.youtube.com/watch_videos?video_ids=<ids in section order>`), over a thumbnail drawn locally in `TutorialsPlaylistThumbnail.tsx`. Sections without a `youtubeId` keep the dashed `.tutorial-media-placeholder` card so deep-links keep working before a video is published. Every link is handed to `window.resmonAPI.openPath`, the preload bridge that ends in `shell.openExternal` in the main process, so it opens in the user's own browser rather than in the app window; the renderer never calls `window.open`.
- The route the media took to get here: animated GIFs first (free-tier converter size limits), then locally bundled `.mp4` files (an ~918 MiB renderer bundle across 17 recordings, with two single files over GitHub's 100 MB limit), then one privacy-enhanced `youtube-nocookie.com` `<iframe>` per section, and now links. The embeds worked; what they cost was a third-party request per video on every visit to the tab, and a merge-gate check that could only be green while somebody else's service was reachable from the CI runner.
...
- Each section emits the following structure:
  ...
  - Either a `.tutorial-media-link` line — the `mediaCaption` text followed by a `Watch on YouTube` link (`data-testid="tutorial-watch-<anchor>"`) to `https://www.youtube.com/watch?v=<id>` — when the section's `youtubeId` is set, or, until a video is published, a `.tutorial-media-placeholder` card with the `mediaCaption` text. No `<iframe>`, `<object>`, `<embed>` or `<webview>` is rendered anywhere in this tab.
