...
- **Name column.** A `Name` column on the Recent Activity table renders the saved-configuration name when the execution row carries one. The cell prefers `e.saved_configuration_name` (joined in by `_enrich_execution_row` from the `saved_configurations` table via the `executions.saved_configuration_id` foreign key), falling back to the row's `routine_name` for routine-fired rows, and finally to `Execution #{e.id}` when neither is available. Manual executions launched from a loaded saved config (Deep Dive / Deep Sweep `ConfigLoader`) and "Save as new config" actions both populate `saved_configuration_id` so the name surfaces here.
