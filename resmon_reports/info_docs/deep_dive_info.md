...
- **AI overrides are opt-in and non-destructive.** The `Override AI settings for this run` disclosure is rendered only when `aiEnabled` is true. Each of `Length`, `Tone`, `Model` defaults to the empty string (`"Use app default"`); `handleRun` drops empty values before sending so they do not clobber persisted settings on the backend merge.
