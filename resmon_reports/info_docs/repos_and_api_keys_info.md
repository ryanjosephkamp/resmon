...
- **Required attributions.** Four sources make a credit a condition of reuse — CORE, OpenAIRE, PLOS and Semantic Scholar — and the page renders those credits above the catalog, unconditionally. Catalog entries carry `attribution`, `attribution_requirement` (`none` / `requested` / `required`) and `attribution_source`; a *requested* credit renders on the source's own detail row instead, so the page never implies resmon is compelled to show something it is not.
