...
## Source coverage and generated companions

Opening a local run shows source coverage above the six existing tabs. **View source details** opens Search record and moves keyboard focus to that tab. Counts use the saved selected-source set only when established; otherwise they describe recorded sources and explicitly leave the full selection unknown. Missing outcome rows remain unknown. Additional recorded sources outside saved selection stay visible separately. Genuine empty answers are a subset of answered sources; rights-filtered or unusable records do not mean no papers were found. Unreadable replies are recorded non-answers.

A failed coverage load offers Retry coverage, without showing an empty successful account. Search record details and its Markdown download identify the execution. The explicit report ZIP includes generated `execution_<id>/search-record.json` and `search-record.md` companions, with a manifest reference and generation timestamp. The original report/log bytes, existing PDF behavior and default routine-mail bundles are unchanged. Old PDFs do not acquire this new account.

Run parameters are requested terms, window and cap, not proof of final provider URLs, exhaustive search or truncation. Source recording timestamps do not prove attempt-start time. This is a read-time account of saved facts, not a historical rewrite or a screening decision.
