...
- **Import** (`handleImport`): iterates `e.target.files`, rejects any file whose name does not end in `.json` with an inline error and a cleared file input. Builds a `FormData` with key `files` per file and POSTs to `/api/configurations/import`. On success reads `data.imported` (and the new `data.routines_created` counter described below) and refreshes the list. The file input value is always reset at the end so the same file can be re-selected.
- **Edit** (`handleEditSave`): the per-row `Edit` button opens a config-type-aware modal with three variants. For `routine` rows the modal pre-populates the full Routines-style form (name + cron + multi-repo selector + date range + keywords + max-results + AI / Email / Email-AI / Notify toggles + Execution-Location radio + the shared `AIOverridePanel`) and saves via `PUT /api/routines/{linked_routine_id}`; on a `404` the handler falls back to `PUT /api/configurations/{id}` with the full routine payload (with `linked_routine_id` cleared) so an orphaned routine config — e.g. one imported from another machine before OOB-11 landed, or one whose linked routine was deleted out-of-band — can still be edited in place. For `manual_sweep` rows the modal renders the same form minus cron and minus the routine-only toggles and saves via `PUT /api/configurations/{id}`. For `manual_dive` rows the modal renders the same form as `manual_sweep` but with `RepositorySelector mode="single"`. After every successful save the handler calls `notifyConfigurationsChanged()` so sibling `ConfigLoader`s and the Routines page refetch. For the `routine` variant specifically, the same cross-page invalidation is also reachable via the Routines page's modal and the Calendar page popover's `Edit Routine` button — both of which use the shared `RoutineEditModal` component, which broadcasts on both the configurations bus and the routines bus (`notifyRoutinesChanged()`) on save. Either path keeps this page's row list and the Routines page's row list in lockstep without a manual reload.
- **View JSON** (`handleViewJson`): the per-row `View JSON` button opens a read-only modal that renders the row's full payload (including its `parameters` JSON, computed `display_name`, and any `linked_routine_id`) inside a `<pre>` block. The modal exposes a Copy-to-clipboard button (uses `navigator.clipboard.writeText` with a `document.execCommand('copy')` fallback) and a Close button; no fetch, mutation, or bus broadcast happens on this path. The button is intended for inspection, debugging, and supporting an exported config side-by-side with its in-app representation.
...
- `POST /api/configurations/import` — accepts a multipart `files: list[UploadFile]`; each upload is written to a temp `.json` file and passed to `config_manager.import_configs`. The handler then post-processes the imported rows: for every config whose `config_type == 'routine'` it parses the inner `parameters` JSON, builds a routine insert payload, **forces `is_active=0`** so imported routines are deactivated by default and bulk imports never auto-fire, calls `insert_routine`, captures the new routine id, and rewrites the just-imported config row's `parameters` JSON so `linked_routine_id` points at the new routine. This restores the bidirectional Routines ↔ routine-configs invariant — every routine config has a real routine row and vice-versa — that the rest of the app already enforces in both other directions (`_sync_routine_config` on routine create/update, and the `delete_routine` cascade on `DELETE /api/configurations/{id}`). Returns `{"imported": n, "routines_created": m, "errors": []}`; per-file JSON-decode failures and `validate_config` failures are still logged server-side and skipped silently, so `imported` may be less than the number of uploaded files, and per-row routine-creation failures (logged and skipped) may make `routines_created` less than the number of imported routine configs.
