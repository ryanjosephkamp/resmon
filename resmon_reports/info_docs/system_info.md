...
- The renderer's CSP `<meta>` tag in `frontend/src/index.html` allows the GitHub Pages origin and nothing else off-machine: `connect-src http://127.0.0.1:* https://ryanjosephkamp.github.io;` (the backend on its chosen loopback port, and the Atom-feed fetch) and `frame-src https://ryanjosephkamp.github.io; child-src https://ryanjosephkamp.github.io;` (the embedded blog webview). The two YouTube origins that were once in `frame-src`/`child-src` for the About resmon → Tutorials tab's per-section `<iframe>` embeds are gone: the tab links to the videos instead, and the directives were narrowed to match. `e2e/renderer-csp.spec.ts` injects a `youtube-nocookie.com` iframe into the real renderer and asserts the policy raises a `frame-src` violation and no request reaches a YouTube host. `font-src 'self' data:` is explained above. All other CSP directives remain unchanged. No additional origins are allowlisted.
