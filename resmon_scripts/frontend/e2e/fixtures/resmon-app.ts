...
export async function selectedEvidenceTransport(root:string,python:string) {
  const {createServer}=await import('http');const {createHash}=await import('crypto');
  ...
  return {cli,backend,control,capture,base:`http://127.0.0.1:${address.port}`,requests,setFault:(fault:string)=>fs.writeFileSync(control,JSON.stringify({fault})),captures:()=>fs.existsSync(capture)?fs.readFileSync(capture,'utf8').trim().split('\n').filter(Boolean).map(line=>JSON.parse(line)):[],receipt:()=>({root,cli,backend,control,capture,apiPort:address.port,cliSha256:createHash('sha256').update(fs.readFileSync(cli)).digest('hex'),backendSha256:createHash('sha256').update(fs.readFileSync(backend)).digest('hex'),keyring:'in-memory synthetic custom key only; no OS keyring access',guard:'unchanged inherited Python sitecustomize'}),close:async()=>{for(const socket of sockets)socket.destroy();await new Promise<void>((resolve,reject)=>server.close(e=>e?reject(e):resolve()));}};
}
