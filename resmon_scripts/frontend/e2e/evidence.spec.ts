...
test('Evidence actual collection, PDF/text passages, notes, restart, exact selected ZIP and isolated refusals',async()=>{
 ...
 const launch=async()=>{
  ...
  const health=await req<{pid:number;started_at:string;identity:{runtime_id:string;schema_version:number}}>('/api/health');expect(health.pid).toBe(backendPid);expect(health.identity.schema_version).toBe(20);
