...
test('Portable briefing: exact saved downloads, stale guards, seven offline states, keyboard, widths and print',async()=>{
 ...
 const launch=async()=>{
  ...
  const health=await req<{pid:number;identity:{runtime_id:string;schema_version:number}}>('/api/health');expect(health.identity.schema_version).toBe(20);runtime=health.identity.runtime_id;backends.push(health.pid);
