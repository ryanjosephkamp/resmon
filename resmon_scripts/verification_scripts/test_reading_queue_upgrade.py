...
def test_upgrading_adds_the_queue_and_changes_nothing_else(legacy):
    ...
    assert database.get_schema_version(legacy) == database.SCHEMA_VERSION
    tables = {r[0] for r in legacy.execute(
        "SELECT name FROM sqlite_master WHERE type='table'")}
    ...
    assert (dict(before["app_settings"]["rows"]) | {"schema_version": str(database.SCHEMA_VERSION)}
...
def test_a_fresh_database_gets_the_same_table_as_an_upgraded_one(tmp_path, legacy):
    ...
    assert database.get_schema_version(fresh) == database.SCHEMA_VERSION
    fresh.close()
...
#: What schemas 14 through 21 add. Everything else in a fresh database must already be in
#: the schema-13 fixture, or the fixture is not schema 13.
SCHEMA_14_TO_18_OBJECTS = {"reading_queue", "idx_reading_queue_status_saved",
                            ...
                            "evidence_answers", "idx_evidence_answers_project_order",
                            # Schema 20: duplicate protection and missed-fire
                            # accounting. The index on ``executions`` counts as
                            # an object schema 20 adds even though the table it
                            # is on is older.
                            "idx_executions_request_id", "routine_missed_fires",
                            "idx_routine_missed_fires_routine",
                            # Schema 21: where a routine's report goes, and
                            # whether it got there.
                            "routine_delivery_targets",
                            "idx_routine_delivery_targets_routine",
                            "deliveries", "idx_deliveries_due",
...
def test_the_fixture_holds_every_object_the_application_owns(legacy, tmp_path):
    ...
    schemas 14 through 20's own objects and what is left is precisely what an upgrading
    ...
    try:
        ...
        expected = {name: kind for name, kind in application_objects(fresh).items()
                    if name not in SCHEMA_14_TO_18_OBJECTS}
    finally:
        fresh.close()
...
def test_the_settings_a_user_had_survive_the_upgrade(legacy):
    ...
    assert after.pop("schema_version") == str(database.SCHEMA_VERSION)
