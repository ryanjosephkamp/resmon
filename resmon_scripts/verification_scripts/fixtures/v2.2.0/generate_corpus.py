#!/usr/bin/env python3
"""Build the committed schema-18 corpus fixture by driving resmon v2.2.0's own code.

This is the v2.1.0 fixture's successor, made under the standing rule in
...
v2.2.0 is the release that took a v2.1.0 user across five schema steps at once
(13 -> 18). Nothing yet migrates past 18, so the walk this fixture serves is
today 18 -> 18: one `init_db` over it must change nothing at all. That is not a
placeholder. `test_a_second_and_third_launch_change_nothing` already says a
second launch changes nothing about a database *this* code upgraded; this says
the same about a database a *released* build wrote, which is the only version of
the claim a user is in.

Usage
-----

    python generate_corpus.py --source-tree /path/to/a/v2.2.0/checkout \\
                              --out corpus_schema_18.sql

    python generate_corpus.py --source-tree ... --check     # regenerate and diff

``--source-tree`` is the root of a checkout of tag ``v2.2.0`` -- normally a
disposable ``git worktree``. The script refuses to run against anything whose
``SCHEMA_VERSION`` is not 18, and refuses to import an ``implementation_scripts``
...
Relationship to `fixtures/v2.1.0/generate_corpus.py`
---------------------------------------------------

This file is a **copy** of that one, extended. The brief allowed the two to
share helpers only if the v2.1.0 fixture still regenerated byte-identically
afterwards; the helpers diverged instead (see *Reproducibility*), so sharing
them would have meant changing the v2.1.0 generator, and its committed fixture
is the one file in the repository that must not move. A copy that can be
diffed against its ancestor was the cheaper honesty.

What it seeds that the schema-13 fixture could not
--------------------------------------------------

Ten tables arrived between 14 and 18, enumerated from the migrations rather
than from memory: `reading_queue` (14), `assistant_session_choices` and
`assistant_turn_choices` (15), `library_vault`, `library_files` and
`library_file_documents` (16), `evidence_projects`, `evidence_project_files`
and `evidence_notes` (17), `evidence_answers` (18). Every one of them is
seeded, at every value of every enumerated CHECK it carries.

One row from the schema-13 fixture is deliberately **not** here: the
`execution_location = 'cloud'` routine. `init_db` rewrites such a row to
`'local'` on open, which is exactly the point of carrying it in a *pre*-2.1.0
corpus -- and exactly what would make this fixture's "one `init_db` changes
nothing" property false. A database v2.2.0 itself wrote cannot contain one.

Reproducibility
---------------

The generator is deterministic modulo the clock **and the identifiers**. Two
normalisations run after seeding, and both are documented on the file:
  ...
  * `_freeze_clock_columns` rewrites every clock column. Schema 13's clock
    ...
    families are frozen into their own shapes rather than one.
  ...
  * `_freeze_identities` rewrites the UUID4s that Library, Evidence, the
    selected-answer lane and the assistant session store mint, and the vault's
    absolute `root_path`, to values derived from where they appear. Without
    this the fixture would differ on every regeneration and could never be
    diffed -- and the root path would carry the generating machine's
...
DEFAULT_OUT = HERE / "corpus_schema_18.sql"

# The schema this generator knows how to seed. A v2.2.0 tree reports exactly
# this; anything else is the wrong tree and the run stops.
EXPECTED_SCHEMA_VERSION = 18

# Clock columns written explicitly by a v2.2.0 writer rather than by a column
...
# ---------------------------------------------------------------------------
# Loading v2.2.0's code
# ---------------------------------------------------------------------------


def load_v220(source_tree: Path):
    """Import v2.2.0's backend modules from ``source_tree`` and sanity-check them."""
    ...
    if already:
        raise RuntimeError(
            ...
            "v2.2.0's copy and nothing else; run it in a fresh interpreter."
        )
    ...
    from implementation_scripts import (assistant_choices, assistant_store, database,
                                        evidence, evidence_reader, library,
                                        reading_queue, watch_profiles)
...
class _Modules:
    """v2.2.0's writers, held together so `seed` takes one argument, not eight."""
...
#   * NULLs in every column v2.2.0 allows one in (a rebuild with a NOT NULL
...
DOCUMENTS = [
    ...
    ("crossref", "10.1000/xyz123", "10.1000/xyz123",
     ...
     "hash-crossref-0001"),
    # Every nullable column at once. v2.2.0 accepts this row; a migration that
...
def seed(m, conn, vault_parent: Path) -> None:
    """Fill a fresh schema-18 database through v2.2.0's own write functions.

    Where v2.2.0 has no writer a generator can reach -- the selected-answer
    ...
    v2.2.0's own constraints and lands in v2.2.0's own column layout, not that
    ...
    for row in DOCUMENTS:
        ...
        if structured:
            # v2.2.0's index_document_facets takes api_base.Author objects. A
        ...
        assert doc_id, f"v2.2.0 refused the document {doc['external_id']}"
    ...
    # test. It is absent here on purpose: v2.2.0's own `insert_routine` refuses
    # that value and its own `init_db` rewrites any survivor, so a corpus
    # v2.2.0 wrote cannot hold one -- and a row today's `init_db` would rewrite
    ...
    # dedup_* have no writer in v2.2.0's database.py -- sweep_engine issues this
    ...
    # watch_profile_members has no writer in v2.2.0's database.py; the API
    ...
    # `CLAUDE_EFFORT_LEVELS` appears in a row v2.2.0 wrote.
    ...
    # `_publish` UPDATEs that follow it), and every row passes v2.2.0's own
...
--
-- Source: tag v2.2.0, commit {commit}
...
def build(source_tree: Path, workspace: Path) -> sqlite3.Connection:
    """Create and seed a schema-18 database under ``workspace``. Returns the connection."""
    started = _dt.datetime.now(_dt.timezone.utc)
    started_sql = started.strftime("%Y-%m-%d %H:%M:%S")
    started_iso = started.isoformat()
    m = load_v220(source_tree)
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--source-tree", required=True, type=Path,
                        help="root of a checkout of tag v2.2.0")
