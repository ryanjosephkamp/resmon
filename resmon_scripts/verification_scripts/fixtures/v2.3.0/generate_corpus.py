...
The tag this fixture belongs to did not exist when it was generated
-----------------------------------------------------------------

v2.2.0's fixture was generated after its tag, from its tag. This one could not
be: `test_the_release_that_shipped_this_schema_left_a_fixture` fails on the
commit that bumps ``APP_VERSION``, which is inside the release PR, and the tag
is only cut once that PR has merged. So this fixture was generated from the
**release branch head** -- the same schema-21 code the tag will carry, since the
squash commit changes no line the dump depends on -- and the header names that
commit and says so. The post-tag follow-up regenerates it from the tag itself,
replaces the commit hash, adds v2.3.0 to CI's tag-fetch loop and sets
``RESMON_REQUIRE_V230_TAG``. Until then
`test_the_committed_fixture_is_what_that_releases_own_code_produces` skips for
v2.3.0, because the tag it would check out is not there.
