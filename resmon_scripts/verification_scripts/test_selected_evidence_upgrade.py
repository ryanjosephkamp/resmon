...
# Schema 19 rebuilds ``executions``: its DDL gains the ``interrupted`` status
# and five nullable columns, and the columns it keeps move to new positions.
# The two helpers below hold this file's claim -- no value rewritten, no row or
# column lost -- without it having to be edited again for the next such step.
REBUILT={'executions'}


def widths(before):
    return {t:before[t]['columns'] for t in before}


def kept(snapshot):
    """A snapshot with the rebuilt tables' column *positions* dropped."""
    return {t:({'columns':[c[1] for c in v['columns']],'rows':v['rows']} if t in REBUILT else v)
...
def test_populated_17_upgrades_all_old_objects_rows_originals_and_notes(legacy17):
    ...
    after=contents(w.conn,set(before),widths(before))
    expected={**before,'app_settings':{**before['app_settings'],'rows':sorted([('schema_version',str(db.SCHEMA_VERSION)) if r[0]=='schema_version' else r for r in before['app_settings']['rows']],key=repr)}}
    assert kept(after)==kept(expected) and files_snapshot(w)==files
    assert ({k:objects(w.conn)[k] for k in old_objects if k not in REBUILT}
            =={k:v for k,v in old_objects.items() if k not in REBUILT})
    assert "'interrupted'" in objects(w.conn)['executions'][2]
    assert len(table_names(w.conn))==40 and db.get_schema_version(w.conn)==db.SCHEMA_VERSION
    ...
    db.init_db(conn=w.conn)
    assert kept(contents(w.conn,set(before),widths(before)))==kept(expected)
    w.conn.close();w.conn=db.get_connection(w.database);db.init_db(conn=w.conn)
    assert kept(contents(w.conn,set(before),widths(before)))==kept(expected) and files_snapshot(w)==files
...
def test_schema18_restart_refuses_corruption_without_repair(legacy17,fault):
    ...
    with pytest.raises(sqlite3.DatabaseError):db.init_db(conn=conn)
    assert objects(conn)==before and contents(conn)==rows and db.get_schema_version(conn)==21
