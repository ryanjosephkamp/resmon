...
def test_populated_17_upgrades_all_old_objects_rows_originals_and_notes(legacy17):
    ...
    expected={**before,'app_settings':{**before['app_settings'],'rows':sorted([('schema_version','19') if r[0]=='schema_version' else r for r in before['app_settings']['rows']],key=repr)}}
    ...
    assert len(table_names(w.conn))==37 and db.get_schema_version(w.conn)==19
...
def test_schema18_restart_refuses_corruption_without_repair(legacy17,fault):
    ...
    with pytest.raises(sqlite3.DatabaseError):db.init_db(conn=conn)
    assert objects(conn)==before and contents(conn)==rows and db.get_schema_version(conn)==19
